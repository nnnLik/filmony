...
export type MutualWatchFriendsMultiPickerProps = {
...
export function MutualWatchFriendsMultiPicker({
  friends,
  loading,
  selectedUserIds,
  onToggle,
}: MutualWatchFriendsMultiPickerProps) {
  return (
    <div className="space-y-2">
      <p className="text-sm font-medium text-(--tgui--text_color)">Кого позвать (необязательно)</p>
      <p className="text-xs text-(--tgui--hint_color)">
        Можно выбрать нескольких взаимных подписок — каждый получит запись в «Позже» и уведомление в
        Telegram.
      </p>
      ...
      {!loading && friends.length > 0 ? (
        <ul className="flex flex-col gap-2">
          {friends.map((row) => {
            ...
            const checked = selectedUserIds.includes(row.id)
            return (
              <li key={row.id}>
                <button
                  type="button"
