...
Пользователь редактирует **свой** видимый профиль (отображаемое имя, био, короткий **slug** для ссылок). Любой **авторизованный** участник открывает **чужой** профиль по внутреннему UUID или по slug и видит список **карточек фильмов** с пагинацией; до появления сущности карточек API возвращает пустой массив с зафиксированным контрактом.
...
Реализовано на бэкенде и фронтенде по контракту v1; карточки и счётчики друзей — заглушки до связанных фич.
