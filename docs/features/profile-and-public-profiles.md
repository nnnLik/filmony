...
- **Клиент:** список полок для блока фильтров не запрашивается, пока пользователь не раскроет «Фильтры и сортировка» (`ProfileRatedCardsFilters`). После успешной загрузки результат кешируется в React Query (**`staleTime` ~15 мин**, дольше живый `gcTime`) и подставляется из **sessionStorage** как placeholder (до 24 ч), чтобы не дёргать сеть без нужды при повторных открытиях вкладки. На вкладке «Статистика» свой профиль использует ту же политику для полок до раскрытия «Подобрать по полям» (`ProfileStatsFilters`). Кеш полок очищается вместе с тегами при сбросе авторизации (`AuthProvider`).
