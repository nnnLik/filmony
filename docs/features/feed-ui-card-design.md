...
## Особая карточка в шапке (`is_favorite`)

- **Цель:** у карточек, отмеченных автором как любимые (`is_favorite`) — заметный, но компактный сигнал **рядом с контекстом ленты** (источник показа), без изменения блока постера и оценки; **одинаково для своих и чужих** карточек в ленте.
- **Реализация:** в [`FeedCard.tsx`](../../frontend/src/components/feed/FeedCard.tsx) — при `is_favorite`: второй бейдж **справа** от mint-бейджа источника (`flex-wrap`, тот же размер шрифта), розовая обводка/фон и подпись **«Особая карточка»**; подсказка при наведении: автор отметил карточку как особую.
