...
## Особая карточка автора в шапке (`is_favorite`)

- **Цель:** у **чужих** карточек с любимым у автора — заметный, но компактный сигнал **рядом с контекстом ленты** (источник показа), без изменения блока постера и оценки.
- **Реализация:** в [`FeedCard.tsx`](../../frontend/src/components/feed/FeedCard.tsx) — у **чужих** карточек с `is_favorite`: второй бейдж **справа** от mint-бейджа источника (`flex-wrap`, тот же размер шрифта), розовая обводка/фон и подпись **«Особая карточка»**; подсказка при наведении: автор отметил карточку как особую. **Свои** карточки в ленте без этого бейджа.
