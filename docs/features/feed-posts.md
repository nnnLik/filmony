...
Если передан `source_comment_id`, комментарий должен принадлежать текущему пользователю; `referenced_movie_card_id` выводится из комментария. Текст поста задаётся только полем `body` (без автоподстановки из комментария). В ответах ленты и `GET /api/feed-posts/{id}` вложенный объект `source_comment` содержит текст и автора исходного комментария для цитаты в UI.
