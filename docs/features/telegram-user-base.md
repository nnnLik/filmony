...
3. Проверить в DevTools / логах, что с фронта уходит `POST …/api/auth/telegram` (и что `VITE_API_ORIGIN` / прокси Vite указывают на живой бэкенд, например `http://127.0.0.1:6749` из `compose.yml`).
