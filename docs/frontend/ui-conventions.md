...
- В раскрытом виде список комментариев **не ограничен превью из ответа ленты**: клиент подгружает **все** комментарии карточки через `GET /api/cards/{id}/comments` (постранично до конца) и показывает их в **фиксированной по высоте** области с **вертикальным скроллом** (`max-h` ≈ прежний визуальный объём ~нескольких сообщений). Поле быстрого комментария остаётся под списком.
...
- [`frontend/src/components/feed/FeedOpenableContainedImage.tsx`](../../frontend/src/components/feed/FeedOpenableContainedImage.tsx) — обёртка с **двойным** открытием fullscreen‑оверлея на вложениях постов и других openable‑поверхностях. Основной **постер** в **`FeedCard`**, **hero** на **`MovieCardDetailPage`** и **большое фото поста** в **`FeedPostCard`** — **на всю ширину карточки**: **`block h-auto w-full max-w-none`** (высота блока = натуральный аспект, **без полос** внутри зоны картинки). Малое превью **ссылочной карточки** в посте по-прежнему в фиксированном боксе **`object-cover`**. Для экранов с фиксированным боксом и **`object-contain`** сохранены опционально **`backdropFill`** + **`FeedContainedImageBackdrop`** — см. [`docs/features/feed-image-viewer.md`](../features/feed-image-viewer.md).
