...
- Прогон в Docker: `make backend-lint`, `make backend-format`. Опционально — [pre-commit](https://pre-commit.com) на `backend/src/` (хуки `ruff-format` + `ruff --fix`; правила — `backend/pyproject.toml`, список хуков — `.pre-commit-config.yaml`; без `--config`, discovery как у Docker): один раз `pre-commit install`; установка и ручной прогон — [`.cursor/tech.md`](/.cursor/tech.md).
