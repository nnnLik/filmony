...
3. **Detail page** (`MovieCardDetailPage.tsx`): mount compact player under the rating overlay on the poster inside a **frosted dark pill** (`.filmony-detail-poster-audio-pill` in `index.css`); pass `compact` to visualizer; remove tags-panel “Аудио к карточке” block entirely.
