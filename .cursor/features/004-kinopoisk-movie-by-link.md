...
> **Статус: реализовано (MVP продукта).** В коде: `POST /api/films/resolve`, `ResolveKinopoiskFilmService`, парсер URL `services/kinopoisk/parse_url.py`, клиент API, фронт `resolveFilmByKinopoiskUrl` / поток создания карточки. Итоговая сводка и необязательные доработки: [`docs/features/kinopoisk-movie-by-link.md`](../../docs/features/kinopoisk-movie-by-link.md).
...
- [x] Valid Kinopoisk film URL returns stable `film_id` and consistent metadata.
- [x] Same URL / same kp id does not duplicate DB rows.
- [x] Invalid URL returns 422 with clear message.
- [x] Poster and title display in UI preview match API payload.
- [x] Secrets for Kinopoisk client live in env, not in repo.

Дополнительно (не блокирует закрытие фичи): расширение парсера под редкие варианты URL; фоновый resolve через Celery при необходимости — см. раздел *Optional enhancements* в `docs/features/kinopoisk-movie-by-link.md`.
