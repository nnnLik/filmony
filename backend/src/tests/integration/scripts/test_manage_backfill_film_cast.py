...
@pytest.mark.asyncio
async def test_films_without_cast_query_selects_rated_only(prepare_db) -> None:
    session = prepare_db
    ...
    film_with_cast = Film(kinopoisk_id=903, title='HasCast', year=2020, poster_url=None, genres=[])
    ...
    assert ids == [film_rated.id]


@pytest.mark.asyncio
async def test_backfill_dry_run_does_not_persist_cast(prepare_db) -> None:
    session = prepare_db
...
@pytest.mark.asyncio
async def test_backfill_runs_ensure_for_candidates(prepare_db) -> None:
    session = prepare_db
    ...
    execute_mock.assert_awaited_once_with(film.id)
